
<%@page import="ModeloDAO.daoProductos"%>
<%@page import="java.util.List"%>
<%@page import="Modelo.Productos"%>
<%

    Productos pro = new Productos();
    daoProductos daoPro = new daoProductos();

    List<Productos> lista = daoPro.Listar();


%>
<table id="example" class=" table table-striped ">

    <thead>
        <tr>
            <th>IdProducto</th>
            <th>Nombre</th>
            <th>Imagen</th>
            <th>Stock</th>
            <th>Precio</th>
            <th>Categoria</th>
            <th>Ultima Actualizacion</th>
            <th>Estado</th>
            <th></th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        <%        for (Productos c : lista) {
                //String cadeimgen = daoPro.getImagen(c.getImagenmostra(), "png").toString();

        %>
        <tr>
            <td><%=c.getIdProductos()%></td>
            <td><%=c.getNombre()%></td>

            <td><img src="${pageContext.request.contextPath}/SevProductosImg?idProducto=<%=c.getIdProductos()%>" alt="alt" width="100px" class="imgProducto"/></td>
            <!--<td><img src="data:image/jpg;base64," alt="alt" class="imgProducto"/></td>-->
            <td><%=c.getStock()%></td>
            <td><%=c.getPrecio()%></td>
            <td><%=c.getCategoria()%></td>
            <td><%=c.getFechaActualiza()%></td>
            <td><span class="status <%=c.getEstado()%>"><%=c.getEstado()%></span></td>
            <td><a href="#"  value="Editar" data-bs-toggle="modal" data-bs-target="#staticBackdrop" onclick="abrirModal('<%=c.getIdProductos()%>', '<%=c.getNombre()%>', '<%=c.getStock()%>', '<%=c.getDescripcion()%>', '<%=c.getPrecio()%>', '<%=c.getIdCategoria()%>')">
                    <img style="width:40px; height: 40px; transform: scale(1.2); cursor: pointer"  src="data:image/png;base64,"/>
                </a>
            </td>
            <td><a href="#" onclick="Eliminar('<%=c.getIdProductos()%>')">
                    <img style="width:40px; height: 40px; transform: scale(1.2); cursor: pointer" src="data:image/png;base64,"/>
                </a>
            </td>
        </tr>
        <%
            }
        %>
    </tbody>
    <tfoot>
        <tr>
            <th>IdProducto</th>
            <th>Nombre</th>
            <th>Imagen</th>
            <th>Stock</th>
            <th>Precio</th>
            <th>Categoria</th>
            <th>Ultima Actualizacion</th>
            <th>Estado</th>
            <th></th>
            <th></th>
        </tr>
    </tfoot>
</table>

