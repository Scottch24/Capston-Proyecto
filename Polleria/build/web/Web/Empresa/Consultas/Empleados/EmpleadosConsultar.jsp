
<%@page import="ModeloDAO.daoEmpleados"%>
<%@page import="Modelo.Empleados"%>
<%@page import="java.util.List"%>

<table id="example" class=" table table-striped" style="width:100%">
    <thead>
        <tr>
            <th>Nombre</th>
            <th>Apellido</th>
            <th>Dni</th>
            <th>Nacimiento</th>
            <th>Telefono</th>
            <th>Correo</th>
            <th>Cargo</th>
            <th>Usuario</th>
            <th>Contraseña</th>
            <th>Ultima Actualizacion</th>
            <th>Estado</th>
            <th></th>
            <th></th>
        </tr>
    </thead>
    <%

        Empleados pro = new Empleados();
        daoEmpleados daoPro = new daoEmpleados();

        List<Empleados> lista = daoPro.Listar();


    %>
    <tbody>
        <%        for (Empleados c : lista) {
        %>
        <tr>
            <td><%=c.getNombres()%></td>
            <td><%=c.getApellidos()%></td>
            <td><%=c.getDni()%></td>
            <td><%=c.getNacimiento()%></td>
            <td><%=c.getTelefono()%></td>
            <td><%=c.getCorreo()%></td>
            <td><%=c.getCargo()%></td>
            <td><%=c.getUsuario()%></td>
            <td><%=c.getContraseña()%></td>
            <td><%=c.getFechaActualizacion()%></td>
            <td><span class="status <%=c.getEstado()%>"><%=c.getEstado()%></span></td>
            <td><a href="#"  value="Editar"  data-bs-toggle="modal" data-bs-target="#staticBackdrop"
                   onclick="abrirModal('<%=c.getIdEmpleado()%>', '<%=c.getNombres()%>', '<%=c.getApellidos()%>', '<%=c.getDni()%>', '<%=c.getNacimiento()%>', '<%=c.getTelefono()%>', '<%=c.getCorreo()%>', '<%=c.getIdCargo()%>', '<%=c.getUsuario()%>', '<%=c.getContraseña()%>')">
                    <img style="width:40px; height: 40px; transform: scale(1.2); cursor: pointer"  src="data:image/png;base64,"/>
                </a>
            </td>
            <td><a href="#" onclick="Eliminar('<%=c.getIdEmpleado()%>')">
                    <img style="width:40px; height: 40px; transform: scale(1.2); cursor: pointer" src="data:image/png;base64,"/>
                </a>
            </td>
        </tr>
        <%
            }
        %>
    </tbody>
</table>