<%@page import="java.util.List"%>
<%@page import="Modelo.Cliente"%>
<%@page import="ModeloDAO.daoCliente"%>
<%
    daoCliente daocli = new daoCliente();
    List<Cliente> lista = daocli.Listar();
%>

<table  id="example" class=" table table-striped" style="width:100%">
    <thead>
        <tr>
            <th>idCLiente</th>
            <th>Nombre</th>
            <th>Apellido</th>
            <th>Genero</th>
            <th>DNI</th>
            <th>Telefono</th>
            <th>Correo</th>
            <th>Usuario</th>
            <th>Contraseña</th>
            <th>Fecha Actualizacion</th>
            <th></th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        <%
            for (Cliente dato : lista) {
        %>
        <tr>
            <td><%=dato.getIdCliente()%></td>
            <td><%=dato.getNombres()%></td>
            <td><%=dato.getApellidos()%></td>
            <td><%=dato.getGenero()%></td>
            <td><%=dato.getDni()%></td>
            <td><%=dato.getTelefono()%></td>
            <td><%=dato.getDireccion()%></td>
            <td><%=dato.getUsuario()%></td>
            <td><%=dato.getContraseña()%></td>
            <td><%=dato.getFecha_Actualizacion()%></td>
            <td><a href="#"  value="Editar"  data-bs-toggle="modal" data-bs-target="#staticBackdrop"
                   onclick="abrirModal(
                               '<%=dato.getNombres()%>',
                               '<%=dato.getApellidos()%>',
                               '<%=dato.getTelefono()%>',
                               '<%=dato.getGenero()%>',
                               '<%=dato.getDni()%>',
                               '<%=dato.getDireccion()%>',
                               '<%=dato.getUsuario()%>',
                               '<%=dato.getContraseña()%>',
                               '<%=dato.getIdCliente()%>',
                               '<%=dato.getEstado()%>'
                               )">
                    <img style="
                         width:40px; 
                         height: 40px; 
                         transform: scale(1.2); 
                         cursor: pointer"  
                         src="data:image/png;base64,"/>
                </a>
            </td>
            <td><a href="#" onclick="Eliminar('<%=dato.getIdCliente()%>')">
                    <img style="width:40px; height: 40px; transform: scale(1.2); cursor: pointer" src="data:image/png;base64,"/>
                </a>
            </td>
        </tr>
        <%
            }
        %>
    </tbody>
</table>

