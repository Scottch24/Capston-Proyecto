
<%@page import="ModeloDAO.daoProductos"%>
<%@page import="java.util.List"%>
<%@page import="Modelo.Productos"%>
<%

    Productos pro = new Productos();
    daoProductos daoPro = new daoProductos();
    String letra = request.getParameter("letra");
    List<Productos> lista = daoPro.Buscar(letra);
%>
<table>
    <thead>
        <tr>
            <th>IdProducto</th>
            <th>Nombre</th>
            <th>Imagen</th>
            <th>Stock</th>
            <th>Precio</th>
            <th>Categoria</th>
            <th>Ultima Actualizacion</th>
            <th>Estado</th>
            <th></th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        <%        for (Productos c : lista) {
        %>
        <tr>
            <td><%=c.getIdProductos()%></td>
            <td><%=c.getNombre()%></td>
            <td><img src="/V2PizzaNapoli/SevProductosImg?idProducto=<%=c.getIdProductos()%>" alt="alt"  class="imgProducto"/></td>
            <td><%=c.getStock()%></td>
            <td><%=c.getPrecio()%></td>
            <td><%=c.getCategoria()%></td>
            <td><%=c.getFechaActualiza()%></td>
            <td><span class="status <%=c.getEstado()%>"><%=c.getEstado()%></span></td>
            <td><a href="#"  value="Editar" onclick="abrirModal('<%=c.getIdProductos()%>', '<%=c.getNombre()%>', '<%=c.getStock()%>', '<%=c.getDescripcion()%>', '<%=c.getPrecio()%>', '<%=c.getIdCategoria()%>')">
                    <img style="width:40px; height: 40px; transform: scale(1.2); cursor: pointer"  src="data:image/png;base64,"/>
                </a>
            </td>
            <td><a href="#" onclick="Eliminar('<%=c.getIdProductos()%>')">
                    <img style="width:40px; height: 40px; transform: scale(1.2); cursor: pointer" src="data:image/png;base64,"/>
                </a>
            </td>
        </tr>
        <%
            }
        %>
    </tbody>
</table>
    
    